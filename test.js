import {
  ...
  export default function CompleteProfileScreen(props) {
    ...
    const handlePress = item => {
      ...
      const isItemExists = foodItems.includes(item);
      if (!isItemExists) {
    ...
    const UploadImage = async imgUri => {
      try {
        ...
        Alert.alert('Error uploading profile pic');
      }
    };
    ...
    const handleRemove = itemToRemove => {
      setfoodItems(prevItems => prevItems.filter(item => item !== itemToRemove));
    };
    ...
    const AgreeButtonContainer2 = () => {
      return (
        <View style={styles.AgreeButtonContainer}>
          <Text style={{color: 'white', fontSize: 12}}>
            Dont worry, you can change your profile name later
          </Text>
          <View
            style={{
              flexDirection: 'row',
            ...
            <View style={{backgroundColor: 'white', margin: 3, borderRadius: 6}}>
              <CheckBox
                disabled={false}
            ...
            <Text style={{color: 'white', fontSize: 12}}>
              I agree to Terms of Service & Policy Privacy
            </Text>
          </View>
          <TouchableOpacity
            onPress={() => (toggleCheckBox == true ? createProfile() : null)}>
            <Text
              style={{
                backgroundColor: toggleCheckBox == true ? '#00BBB4' : 'gray',
                padding: 10,
                borderRadius: 10,
              }}>
             Complete Profile
          ...
          <Text style={{color:'white',fontSize:15,marginTop:5}}>{'Skip'}</Text>
          </TouchableOpacity>
        </View>
      );
    };
